...
class ShowcaseExampleViewController: UIViewController, UITextFieldDelegate {
    ...
    func setupThemeColors() {
        ...
        if isLTRLanguage {
            self.arrivalCityField.iconRotationDegrees = 90
                      self.arrivalCityField.lineHeight = 0.0
            self.arrivalCityField.selectedLineHeight = 0.0
    ...
    func showingTitleInAnimationComplete() {
        ...
        let displayTime: DispatchTime = DispatchTime.now() + Double(Int64(0.3 * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)
        DispatchQueue.main.asyncAfter(deadline: displayTime, execute: {
